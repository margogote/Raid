...
public class Inte_Acquisition_Modif extends JFrame {
	...
	public void Interface() {
		...
		gigaP.setBorder(BorderFactory
				.createTitledBorder("Ici vous pouvez modifier une acquisition de votre épreuve"));
		...
		String requeteSQL = "SELECT `equipe`.nomEquipe, pointer.`dateHeurePointage`, pointer.`idBalise` FROM `pointer` INNER JOIN (`posséder` INNER JOIN `equipe` ON `posséder`.idEquipe = `equipe`.idEquipe) ON `pointer`.`idDoigt` = `posséder`.`idDoigt` WHERE `pointer`.`idCompetition` = '"
				+ idc
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (nom.equals("CHOISIR") || date.equals("")
				...
				String requeteSQL2 = "UPDATE `pointer` SET `idBalise` = '"
						+ balise
						+ "', `idDoigt` = (SELECT `idDoigt` FROM `posséder` WHERE `idEquipe`='"
						+ ideqN
						+ "'), `dateHeurePointage` = '"
						+ date
